    ...
    T: Eq + Hash + Clone,
{
    ...
    fn add_tag(&mut self, tag: T) {
        ...
        let version_num = self.add_tags.get(&tag).expect("could not extract value").0;             //this way, the version number for the same tag 
        ...
        else if self.add_tags.get(&tag).expect("could not extract value").0 == self.remove_tags.get(&tag).expect("could not extract value").0 {
            let val = self.add_tags.get_mut(&tag).expect("could not extract value");
    ...
    fn remove_tag(&mut self, tag: T) {
        ...
        let version_index_to_be_deleted: usize = (self.add_tags.get(&tag).expect("could not extract value").0 - 1) as usize;
        let version_to_be_deleted = self.add_tags.get(&tag).expect("could not extract value").1[version_index_to_be_deleted];

        if !self.remove_tags.contains_key(&tag) {
            let val = self.remove_tags.get_mut(&tag).expect("could not extract value");
            val.0 = 1;
